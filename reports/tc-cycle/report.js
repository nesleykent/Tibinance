...
async function main() {
  ...
  <header class="hero">
    ...
    <p class="deck">Estudo de 16 mundos sobre Sell Offers e Buy Offers: cenários condicionais, model-implied probabilities, relative premium e execution cost. Piece Price em gold por Tibia Coin (gp/TC).</p>
  ...
  <section class="market-panel" aria-labelledby="market-title">
    <div class="market-heading"><div><p class="section-kicker">Market monitor <span>16 worlds / ${U.captureCount} captures</span></p><h2 id="market-title">Preços observados por mundo</h2></div><p>Última captura disponível · gp/TC<br>Variação desde a captura anterior do mesmo mundo</p></div>
    ${table({columns: [
  ...
  <section class="block" id="sumario">
    ...
    <div class="prose">
      ...
      <ul>
        ...
        <li><strong>Model-implied probabilities são sensíveis ao recorte histórico e à captura inicial.</strong> Reestimado sobre Sell Offers de Antica, o modelo atribui ${prob(m23.prob.peakAbove3)} (treino desde 2023) a ${prob(m24.prob.peakAbove3)} (desde 2024) ao evento “máximo até 28/02/2027 pelo menos 3% acima da captura”. Trocar a semente altera a estimativa em até ${seedSpread} p.p. (${fmt(pkgSeedSpread * 100, 1)} p.p. no <code>forecast_stability.json</code> do pacote); trocar a amostra, em até ${sampleGap} p.p.; trocar a captura inicial (21, 22 ou 23/09), em até ${anchorGap} p.p. Apenas o ajuste desde 2023 teve out-of-sample backtest; em 13 semanas, sua previsão de direção perdeu para a seasonal baseline.</li>
        <li><strong>O sell-now / buy-back apresenta risco material de execução e de cenário.</strong> Aceitando Buy Offers na venda e Sell Offers na semana de 28/06/2027 para recomprar, há ganho em TC em ${prob(rt('main', 'aceitando').pGain)} das trajetórias simuladas com treino desde 2023 e em ${prob(rt('since2024', 'aceitando').pGain)} com treino desde 2024. Essas proporções descrevem o modelo, não uma frequência de sucesso demonstrada.</li>
        <li><strong>A combinação de modelos agrega informação em Antica, mas não em todos os mundos.</strong> Em 13 semanas, o erro médio do preço em Sell Offers foi ${fmt(summaries[0].ensemble, 1)}%, ante ${fmt(summaries[0].constant, 1)}% com preço constante. No horizonte anual há apenas ${summaries.find(x => x.horizon === 52)?.n} origens para cada lado do Market: pouco para julgar os modelos em 52 semanas.</li>
        ...
        <li><strong>Quatro mundos exibem relative premium nas ofertas, menor no corte de 23/09.</strong> Belobra, Celebra, Gentebra e Luminera, todos Optional PvP com BattlEye Yellow, ficaram cerca de ${sgn(yA.recentMedianPct, 0)} acima de Antica em Sell Offers nas últimas 26 semanas; os demais grupos, entre ${sgn(Math.min(...others.map(g => g.recentMedianPct)))} e ${sgn(Math.max(...others.map(g => g.recentMedianPct)))}. Nas oito semanas até ${brShort(luzLast)}, o prêmio do grupo caiu para ${sgn(yA.last8MedianPct, 0)}; nas capturas de 21–23/09, ${sgn(yA.currentMedianPct, 0)} em Sell Offers e ${sgn(yB.currentMedianPct, 0)} em Buy Offers. É um padrão observado de quatro mundos, sem inferência de preço executado.</li>
  ...
  <section class="block" id="s02">
    ...
    <div class="prose">
      <p>O ensemble combina uma baseline de preço constante, repetição sazonal de 52 semanas e regressão com tendência e harmônicos anuais. O treinamento usa <strong>ofertas históricas de Antica</strong>, ancoradas na captura de 23/09. Sell Offers e Buy Offers são modeladas separadamente.</p>
      <p>Para Sell Offers de Antica, o base case é <strong>${price(nov.base)} gp/TC</strong> em novembro, com downside/upside stress de <strong>${price(nov.low)} a ${price(nov.high)}</strong>. Em junho, o base case é <strong>${price(june.base)}</strong>, com stress range de <strong>${price(june.low)} a ${price(june.high)}</strong>. As faixas combinam erro histórico e divergência entre modelos; <strong>não são P10/P90 nem confidence intervals calibrados</strong>. A seção 04 apresenta separadamente a distribuição simulada e seu backtest.</p>
    </div>
  ...
  <section class="block" id="s08">
    ...
    <div class="prose"><p>A perda de comprar e revender imediatamente (1 − Buy Offers / Sell Offers) nas capturas de 21 a 23/09 (a mais recente de cada mundo; Celebra: 21/09) é comparada às leituras da API dos 180 dias anteriores a 23/09. ${wideNow.length ? `Em ${list(wideNow.map(w => `${w.world} (${pctU(w.nowPct, 2)} contra mediana de ${pctU(w.recentMedianPct, 2)})`))}, a diferença na captura da pesquisa supera em mais de 50% a habitual e representa um round-trip execution cost acima da mediana histórica desses mundos. A comparação é indicativa, pois as capturas e as leituras da API não são simultâneas.` : ''} ${terWide ? `Terribra, sem captura, também aparece em destaque pela última leitura da API (${pctU(terWide.nowPct, 2)} em ${br(terWide.nowDate)}, contra mediana de ${pctU(terWide.recentMedianPct, 2)}).` : ''} Em Antica, a diferença no corte da pesquisa (${pctU(antica.costPct, 2)}) está abaixo da habitual (${pctU(C.spread.worlds.find(w => w.world === 'Antica').recentMedianPct, 2)}). Cada captura é uma leitura única, em horário diferente das leituras da API; não indica quanto tempo a condição dura. Por mês do ano, descontado o nível de cada ano, a variação em Antica fica entre ${pp(Math.min(...monthFx), 2)} e ${pp(Math.max(...monthFx), 2)}.</p></div>
    <div id="card-spread"></div>
    <h3>Weekday effect: sem sinal robusto nas ofertas</h3>
    <div class="prose"><p>O pacote recebido encontrou diferenças por dia da semana em médias diárias cuja ponderação não está documentada. Nesta edição, a medida usa somente Sell Offers e Buy Offers: desvio de cada dia do servidor em relação à mediana dos sete dias ao redor. Em Antica, a diferença entre o dia mais caro e o mais barato é de ${pctU(wA.rangePct, 2)} em Sell Offers (p = ${fmt(wA.pBlock, 2)} permutando dentro de cada semana) e ${pctU(wAb.rangePct, 2)} em Buy Offers (p = ${fmt(wAb.pBlock, 2)}); após correção dos quatro testes, nenhum p fica abaixo de ${fmt(Math.min(...C.weekday.tests.map(t => t.pHolm)), 2)}. Mesmo a maior diferença equivale a ${fmt(wAb.rangePct / 100 * antica.bid)} gp/TC, menos que o quoted spread e bem menos que a taxa de 2% de Create Offer. Os mundos BR raramente têm cotações em dias seguidos (mediana de 1 a 2 dias por semana em ${sparse.length} dos ${cov.length} mundos; ${list(dense.map(c => `${fmt(c.medianDaysPerWeek)} em ${c.world}`))}); com uma janela menos exigente (3 de 7 dias), ${Object.keys(wB.contributors).length} mundos somam ${fmt(wB.n)} dias, com p = ${fmt(wB.pBlock, 2)} e ${fmt(wBb.pBlock, 2)}. Nos BR, a amostra é limitada; não há evidência suficiente para escolher o dia da execução pelo calendário semanal.</p></div>
  ...
  <section class="block" id="s09">
    ...
    <div class="prose">
      ...
      <p><strong>No corte da pesquisa.</strong> Da semana de 30/08 à captura de 23/09 (leitura única), as Sell Offers de Antica subiram ${sgn(mo.anchorPast4Pct)}, logo acima do limiar das 20% maiores altas de quatro semanas (${sgn(mo.thresholdUpPct)}); até a última cotação da API (${br(mo.historicalWeek)}), ${sgn(mo.historicalPast4Pct)}, abaixo dele.</p>
    </div>
  ...
  <section class="block" id="s13">
    ...
    <div class="prose">
      <p><strong>Universo e relógio.</strong> O painel cobre os 16 mundos solicitados. O preço histórico usa sell_offer e buy_offer, com dia do servidor iniciado às 10h de Europe/Berlin. As capturas preservam a data fornecida, sem presumir um fuso ausente do arquivo. O Market monitor incorpora capturas até 25/09; pesquisa, backtests e cenários têm corte em 23/09. A última oferta histórica da API varia por mundo. A data de execução da coleta não substitui a data da cotação. As medianas semanais cobrem de segunda a domingo e são rotuladas pelo domingo; os cenários da seção 02 avançam de sete em sete dias a partir da captura de 23/09 (quartas-feiras).</p>
      <p><strong>Higienização.</strong> Ausências, preços não positivos, quadros de ofertas cruzados e observações em que o preço em Buy Offers fica abaixo de 80% do preço em Sell Offers saem da modelagem. Esse último critério retira preços extremos, inclusive compras de 1 gp, mas também pode retirar quadros de ofertas reais muito abertos. Os dados brutos são preservados. Não há preenchimento de lacunas com médias diárias nem filtro de mediana centrado que consulte o futuro.</p>
  ...
  const NOTES = {
    ...
    Belobra: 'A market depth visível concentra-se em Buy Offers, mas o Amount não mede TC negociadas. O backtest da sazonalidade transferida ficou atrás da constant-price baseline; trate a trajetória cíclica como sensibilidade.',
    ...
    Gentebra: 'O Amount concentrado em Buy Offers coincide com quoted spread amplo. Criar uma Sell Offer não garante execução. Para vender TC imediatamente, a referência é o melhor Piece Price em Buy Offers e seu Amount disponível.',
    ...
    Ourobra: 'O quoted spread está entre os menores do painel. Isso reduz o round-trip execution cost implícito, mas o volume executável depende do Amount no melhor Piece Price.',
    ...
    Venebra: 'A razão entre Amount em Buy Offers e Sell Offers é a maior da amostra, mas o quoted spread permanece amplo. A market depth do lado comprador, isoladamente, não indica que Sell Offers serão aceitas.',
  };
  document.getElementById('dossiers').innerHTML = worlds.map(w => {
    ...
    return `<article class="dossier" data-world="${w.world}" id="dossier-${w.world.toLowerCase()}">
      ...
      <p class="dossier-current"><strong>Última leitura · ${br(q.capturedAt)}${q.capturedAt.length > 10 ? ` · ${q.capturedAt.slice(11, 16)}` : ''}.</strong> Sell Offers ${fmt(q.sell)} gp/TC (${change(u.deltaPct.sell)}); Buy Offers ${fmt(q.buy)} gp/TC (${change(u.deltaPct.buy)})${u.prior ? ` ante ${brShort(u.prior.capturedAt)}` : '; sem comparação anterior'}. ${q.sellTopAmount == null ? 'Sem captura recente de Amount.' : `Amount no melhor Piece Price: ${fmt(q.sellTopAmount)} TC em Sell Offers; ${fmt(q.buyTopAmount)} TC em Buy Offers.`}</p>
      <p><strong>Research baseline · ${br(R.asOf)}.</strong> Buy Offers ${fmt(w.bid)}; Sell Offers ${fmt(w.ask)} gp/TC; quoted spread ${fmt(w.spreadPct, 2)}%. Contra a mediana das ofertas disponíveis nos 90 dias anteriores, Buy Offers ${sgn(w.bidVs90)} e Sell Offers ${sgn(w.askVs90)}. ${w.buyVolume == null ? 'Sem captura recente de market depth.' : `Market depth: ${price(w.buyVolume)} TC em Buy Offers e ${price(w.sellVolume)} TC em Sell Offers. No melhor Piece Price: ${fmt(w.buyTopAmount)} TC em Buy Offers e ${fmt(w.sellTopAmount)} TC em Sell Offers.`}</p>
      <p><strong>Base case e backtest.</strong> ${w.world === 'Luzibra' ? 'Após a fusão, avaliar as primeiras Sell Offers e Buy Offers de Deslumbra antes de atribuir níveis numéricos ou probabilidades.' : milestones + ' gp/TC no base case.'} ${w.testN ? `${w.world === 'Antica' ? 'O backtest do ensemble (horizontes de 4 a 52 semanas, os dois lados do Market)' : 'O backtest da transferência'} apresenta erro médio de ${fmt(w.testMape, 1)}%, contra ${fmt(w.testNaive, 1)}% na constant-price baseline (${w.testN} comparações sobrepostas de lado e horizonte).` : 'Sem observações suficientes para testar a transferência.'} ${w.testMape > w.testNaive ? 'Como o modelo não supera a baseline, a trajetória cíclica é apenas uma análise de sensibilidade.' : ''} Cobertura: ${fmt(w.days)} dias, de ${br(w.first)} a ${br(w.last)}; ${w.captureCount === 0 ? 'sem capturas' : w.captureCount === 1 ? '1 captura' : `${w.captureCount} capturas`} em 21–23/09.</p>
      ${pa && ok(pa.last8PremiumPct) ? `<p><strong>Relative premium.</strong> Sell Offers ante Antica: ${sgn(pa.recentPremiumPct)} nas últimas 26 semanas, ${sgn(pa.last8PremiumPct)} nas últimas 8 e ${sgn(pa.currentPremiumPct)} nas capturas.${w.world === 'Luzibra' ? ` Medido só desde a mudança de patamar de ${brShort(pa.breakWeek)}; antes, desconto de ${fmt(Math.abs(pa.preBreakMedianPct))}% (${fmt(Math.abs(pa.preBreak8Pct))}% nas oito semanas anteriores).` : ''}</p>` : ''}
    </article>`;
  }).join('');
