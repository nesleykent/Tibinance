...
async function main() {
  ...
  let html = `
  <header class="hero">
    <p class="eyebrow">Tibinance Research · Cycle outlook 2026–2027 · Revisão editorial de 25/09/2026</p>
    ...
    <p class="meta">Market data as of ${br(R.asOf)} · Fontes: <a href="https://api.tibiamarket.top/docs">API pública do TibiaMarket</a> e 45 capturas do Market · <a href="https://github.com/nesleykent/Tibinance/tree/main/reports/tc-cycle">método, dados e reprodução</a></p>
  </header>
  <div class="kpis" role="list">
    ...
    <div class="kpi" role="listitem"><div class="label">Model-implied probability · pico ≥ +3% até fev. 2027</div><div class="value">${prob(m23.prob.peakAbove3)} a ${prob(m24.prob.peakAbove3)}</div><div class="note">Sell Offers de Antica. Sensível à amostra de treino e à captura inicial (${anchorLo}–${anchorHi}%); apenas o ajuste desde 2023 teve backtest</div></div>
    <div class="kpi" role="listitem"><div class="label">Sell now / buy back · junho de 2027</div><div class="value">${prob(rt('main', 'aceitando').pGain)} a ${prob(rt('since2024', 'aceitando').pGain)}</div><div class="note">Parcela das trajetórias simuladas com ganho em TC ao aceitar Buy Offers agora e Sell Offers na recompra; sujeita a execution risk</div></div>
  </div>
  ...
  <section class="block" id="s01">
    ...
    ${card({title: 'Ofertas mais recentes por mundo', sub: 'gp/TC · capturas de 21 a 23/09; Terribra: último histórico', body: table({columns: [
      {key: 'world', label: 'Mundo'}, {key: 'date', label: 'Data', render: br}, num('bid', 'Buy Offers'), num('ask', 'Sell Offers'), num('spreadPct', 'Quoted spread %', 2),
      {key: 'depthRatio', label: 'Amount: Buy / Sell', num: true, render: v => ok(v) ? `${fmt(v, 2)}×` : '—'}, num('ageDays', 'Defasagem (dias)')], rows: worlds})})}
    <div class="prose">
      ...
      <p><strong>Piece Price</strong> é o preço por TC; <strong>Amount</strong>, a quantidade ofertada. <strong>Market depth</strong> é o Amount total visível em cada lado do Market; não equivale ao Amount disponível no melhor Piece Price, nem a TC efetivamente negociadas. A razão Amount: Buy / Sell compara essas quantidades totais. As capturas não são simultâneas: Celebra está em 21/09 e os demais mundos capturados em 23/09. A idade é medida em relação ao corte de 23/09.</p>
